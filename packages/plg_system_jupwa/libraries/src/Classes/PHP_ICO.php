...
class PHP_ICO
{
    ...
    {
        if (!$this->_has_requirements || ($data = $this->_get_ico_data()) === false || ($fh = fopen(
                $file,
                'wb'
            )) === false) {
            return false;
     ...
     * @since  1.0
     */
    public function _add_image_data(GdImage $im): void
    {
        $width = imagesx($im);
        $height = imagesy($im);
        ...
        $opacity_mask_size = (int)(ceil($width / 32) * 4) * $height;
     ...
     * @since  1.0
     */
    public function _load_image_file(string $file): GdImage|false
    {
        ...
        return imagecreatefromstring($file_data);
    }
}
